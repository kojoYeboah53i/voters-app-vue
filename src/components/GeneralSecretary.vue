...
<div class="row mb-5">
   <div class="col-12 ">
       <div class="row justify-content-center">
           <div class="col-12">
               <h2 class="text-center">General Secretary Election</h2>
...
export default {
    ...
    methods: {
        submitVote(data) {
            ...
            const presidents = JSON.parse(localStorage.getItem('general_secretaries'))
            
            // check whether the user is already voted
            if(!user) {
            ...
            if(!localStorage.getItem('general_secretaries_voters')) {
                ...
                setTimeout(function(){
                    ...
                    setTimeout(function() {
                        localStorage.setItem('currentPage','financial_secretary')
                        self.$router.replace('/vote/financial_secretary')
                    },3000)

                },1000)
            } else {
                const oldVoters = JSON.parse(localStorage.getItem('general_secretaries_voters'))
                const newArray = [user]
                ...
                localStorage.setItem('general_secretaries_voters', JSON.stringify(newVoters))
